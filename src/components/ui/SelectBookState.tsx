...
export default function SelectBookState() {
    ...
    return (
        <div className="flex flex-col w-[200px] relative">
            <button
                type="button"
                className="flex items-center justify-between w-full bg-primary py-2 px-4 rounded-md transition-colors hover:bg-primary/90"
                onClick={showBookState}
            >
                <div className="flex items-center gap-x-4">
                    {getIconForSelectedState(selectedState)}
                    <p className="font-bodyFont font-semibold text-primary-foreground">
                        {selectedState}
                    </p>
                </div>
            ...
            <AnimatePresence>
                {openBtn && (
                    ...
                    >
                        {bookState.map((state) => (
                            <motion.div
                                className={`flex items-center gap-x-4 py-2 cursor-pointer text-card rounded-md transition-all duration-200 ease-in-out hover:bg-card-foreground hover:text-card hover:pl-4 ${
